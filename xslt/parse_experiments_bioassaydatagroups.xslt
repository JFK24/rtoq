...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs">
	...
	<xsl:template name="process_bioassaydatagroup">
		...
		<xsl:text>&#x9;</xsl:text>
		<xsl:value-of select="bioassaydatacubes" />
		<xsl:text>&#x9;</xsl:text>
		<xsl:value-of select="arraydesignprovider" />
		<xsl:text>&#x9;</xsl:text>
		<xsl:value-of select="dataformat" />
		<xsl:text>&#x9;</xsl:text>
		<xsl:value-of select="bioassays" />
		<xsl:text>&#x9;</xsl:text>
		<xsl:value-of select="isderived" />
	</xsl:template>

	<xsl:template match="experiment">
		<xsl:for-each select="bioassaydatagroup">
			<xsl:call-template name="process_bioassaydatagroup">
